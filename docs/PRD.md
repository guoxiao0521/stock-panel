...
日期：2026-06-14  
项目类型：个人自用美股自选股分析工具  
技术栈：Nuxt 4、Vue 3、TypeScript、Tailwind CSS、shadcn-vue、Supabase Postgres、yahoo-finance2

## 1. 背景

Stock Panel 是一个面向个人投资者自用的美股观察与分析面板。第一阶段聚焦自选股管理和关键行情指标展示，后续逐步扩展到宏观市场状态监控和 AI 辅助分析。

项目当前是 Nuxt 4 工程，前端界面优先使用 shadcn-vue 组件，样式体系使用 Tailwind CSS。数据源第一阶段以 `yahoo-finance2` 为主，业务数据和缓存数据落到 Supabase Postgres。
...
- 使用 Supabase Postgres 保存自选股列表、备注、标签、行情快照和更新时间。
...
| 阶段 | 名称 | 重点 | 交付结果 |
| --- | --- | --- | --- |
| Phase 1 | 自选股面板 | 主框架、自选股、行情指标、Postgres | 可用的自选股管理和行情展示 |
...
| 净值 | navPrice | yahoo-finance2（ETF） |
| 折溢价 | (最新价 - 净值) / 净值 * 100 | 服务端计算（ETF） |
| 标签 | 用户维护 | Postgres |
| 备注 | 用户维护 | Postgres |
| 更新时间 | 最新行情快照时间 | Postgres |
...
如果后续需要 ORM，可以再评估 Drizzle ORM 或 Prisma。第一阶段为了速度和可控性，可先直接封装 Postgres repository 层。

### 10.2 数据刷新策略

- 页面首次加载时读取 Postgres 中的自选股和最近行情快照。
...
- ETF 折溢价：
  - 优先从 `quote.navPrice` 读取净值；缺失时对 ETF 使用 `quoteSummary.summaryDetail.navPrice`。
  - 使用 `(regularMarketPrice - navPrice) / navPrice * 100` 计算。
  - 非 ETF 或 NAV 缺失时展示 `-`。
...
| nav_price | real | ETF 净值 |
| premium_discount_percent | real | ETF 折溢价百分比 |
| quote_time | text | 行情时间 |
| fetched_at | text | 拉取时间 |
| source | text | 数据源 |
| error | text | 最近错误 |
| raw_json | jsonb | 原始数据 JSON，便于调试 |
...
- 数据库连接失败：页面展示明确错误，开发环境输出详细日志。
- 指标缺失：展示 `-`，不抛前端异常。

## 15. 性能要求

- 20 到 50 只自选股时，页面交互应保持流畅。
- 首屏优先展示 Postgres 缓存数据，再异步刷新远端行情。
- 批量刷新应限制并发，避免被数据源限流。
- 表格搜索和排序在前端完成即可。
...
### Milestone 2：Postgres 数据层

- 添加 Postgres 依赖。
- 通过 Supabase migration 初始化数据库和表结构。
